...
void DiodeClipperAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    diodeClipper[0].prepare(sampleRate);
    diodeClipper[1].prepare(sampleRate);

    LCO[0].prepare(sampleRate);
    LCO[1].prepare(sampleRate);
}
...
void DiodeClipperAudioProcessor::releaseResources()
{
    ...
    // spare memory, etc.
    diodeClipper[0].reset();
    diodeClipper[1].reset();
